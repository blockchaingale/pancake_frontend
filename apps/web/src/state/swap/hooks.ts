...
import { MMTOKEN, USDC, USDT } from '@pancakeswap/tokens'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    if (!MMTOKEN[chainId] && !USDT[chainId]) return
    const parsed = queryParametersToSwapState(query, native.symbol, MMTOKEN[chainId]?.address ?? USDT[chainId]?.address)
