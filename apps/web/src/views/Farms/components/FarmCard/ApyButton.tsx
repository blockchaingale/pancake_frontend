...
const ApyButton: React.FC<React.PropsWithChildren<ApyButtonProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text ml="5px" small>
        *{t('Base APR (MM yield only)')}:{' '}
        {strikethrough ? (
          <Text style={{ display: 'inline-block' }} color="secondary" small>{`${(apr * boostMultiplier).toFixed(2)}%`}</Text>
        ) : (
          `${apr.toFixed(2)}%`
        )}
      </Text>
      ...
      {strikethrough && (
        <Text small>
          {t('Available Boosted')}:{' '}
          <Text color="secondary" style={{ display: 'inline-block' }} small>
            {t('Up to %boostMultiplier%x', { boostMultiplier: boostMultiplierDisplay })}
          </Text>
        </Text>
      )}
      {strikethrough && <Text color="secondary">{t('Boost only applies to base APR (MM yield)')}</Text>}
    </>,
    {
      placement: 'top',
    },
