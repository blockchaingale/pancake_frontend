...
const StakeAction: React.FC<React.PropsWithChildren<FarmCardActionsProps>> = ({
  ...
  const crossChainWarningText = useMemo(() => {
    return isFirstTime
      ? t('A small amount of %nativeToken% is required for the first-time setup of cross-chain MM farming.', {
  ...
  if (!isApproved) {
    return (
      <Button mt="8px" width="100%"  onClick={handleApprove} disabled={true}>
