...
export default function CommonBases({
  ...
  const THIRD_LINE_ETH = [SUGGESTED_BASES[chainId][5], SUGGESTED_BASES[chainId][6], SUGGESTED_BASES[chainId][7]]
  ...
  return (
    <AutoColumn gap="md">
      ...
      <RowWrapper>
        {(chainId === ChainId.ARBITRUM || chainId === ChainId.BSC ? THIRD_LINE_ETH || [] : []).filter((token: Token) => token !== undefined).map((token: Token) => {
          console.log('selectedCurrency', selectedCurrency, 'token', token)
          const selected = selectedCurrency?.equals(token)
          return (
            <ButtonWrapper key={`buttonBase#${token.address}`}>
              <BaseWrapper onClick={() => !selected && onSelect(token)} disable={selected}>
